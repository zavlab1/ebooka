...
public class DocumentView extends View implements ZoomListener {
	...
	public void init(final ZoomModel zoomModel,
			DecodingProgressModel progressModel,
			CurrentPageModel currentPageModel, View menuView) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		if (multiTouchZoom != null
				&& menuViewLayout.getVisibility() == View.VISIBLE) {
	...
	protected void onDraw(Canvas canvas) {
		...
		pages.get(visible).draw(canvas);
		if (visible > 1) {
			pages.get(visible - 1).draw(canvas);
		}
		if (visible + 1 < pages.size()) {
			pages.get(visible + 1).draw(canvas);
		}
	}
